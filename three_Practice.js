...
// import позволяет выбрать, какие части файла или модуля загружать. В предыдущем уроке примеры экспортировались addиз math_functions.jsфайла. Вот как вы можете импортировать его для использования в другом файле:

// import { add } from './math_functions.js';
// Здесь importвы найдете add, math_functions.jsимпортируете только эту функцию для использования и проигнорируете остальные. Сообщает ./импорту искать math_functions.jsфайл в той же папке, что и текущий файл. Относительный путь к файлу ( ./) и расширение файла ( .js) необходимы при использовании импорта таким образом.

// Вы можете импортировать более одного элемента из файла, добавив их в importоператор следующим образом:

// import { add, subtract } from './math_functions.js';
// Добавьте соответствующий importоператор, который позволит текущему файлу использовать функции uppercaseStringи lowercaseString, которые вы экспортировали в предыдущем уроке. Эти функции находятся в файле с именем string_functions.js, который находится в том же каталоге, что и текущий файл.
...
// Предположим, у вас есть файл, и вы хотите импортировать все его содержимое в текущий файл. Это можно сделать с помощью import * asсинтаксиса. Вот пример, когда содержимое указанного файла math_functions.jsимпортируется в файл в том же каталоге:

// import * as myMathModule from "./math_functions.js";
// Приведенный выше importоператор создаст объект с именем myMathModule. Это просто имя переменной, вы можете назвать его как угодно. Объект будет содержать все экспорты из math_functions.jsнего, поэтому вы можете получить доступ к функциям, как и к любому другому свойству объекта. Вот как можно использовать импортированные функции addи subtract:

// myMathModule.add(2,3);
// myMathModule.subtract(5,3);
// Для кода в этом файле требуется содержимое файла: string_functions.js, который находится в том же каталоге, что и текущий файл. Используйте import * asсинтаксис для импорта всего из файла в объект с именем stringFunctions.
...
// На export уроке вы узнали о синтаксисе, называемом именованным экспортом . Это позволило сделать несколько функций и переменных доступными для использования в других файлах.

// Есть еще один export синтаксис, который вам нужно знать, известный как экспорт по умолчанию . Обычно вы будете использовать этот синтаксис, если из файла экспортируется только одно значение. Он также используется для создания резервного значения для файла или модуля.
...
// Поскольку export default используется для объявления резервного значения для модуля или файла, вы можете иметь только одно значение, которое будет экспортом по умолчанию в каждом модуле или файле. Кроме того, вы не можете использовать export default с var, let или const
...
// В последнем испытании вы узнали о export default его использовании. Чтобы импортировать экспорт по умолчанию, вам нужно использовать другой importсинтаксис. В следующем примере add это экспорт math_functions.js файла по умолчанию. Вот как его импортировать:

// import add from "./math_functions.js";
// Синтаксис отличается одним ключевым местом. Импортированное значение addне заключено в фигурные скобки ( {}). add здесь просто имя переменной для любого экспорта math_functions.jsфайла по умолчанию. Вы можете использовать здесь любое имя при импорте значения по умолчанию.

// В следующем коде импортируйте экспорт по умолчанию из math_functions.jsфайла, который находится в том же каталоге, что и этот файл. Дайте импорту имя subtract.
...
// Обещание в JavaScript звучит именно так: вы используете его, чтобы дать обещание что-то сделать, обычно асинхронно. Когда задача завершается, вы либо выполняете свое обещание, либо не выполняете его. Promiseявляется функцией-конструктором, поэтому для ее создания необходимо использовать newключевое слово. В качестве аргумента принимает функцию с двумя параметрами - resolve и reject. Это методы, используемые для определения результата обещания. Синтаксис выглядит следующим образом:

// const myPromise = new Promise((resolve, reject) => {

// });
// Создайте новое обещание с именем makeServerRequest. Передайте функцию с параметрами resolveи reject в конструктор.
...
// Промис имеет три состояния: pending, fulfilled, и rejected. Обещание, которое вы создали в последнем испытании, навсегда застряло в pendingсостоянии, потому что вы не добавили способ выполнить обещание. Для этого используются параметры resolveи reject, указанные в аргументе обещания. resolve используется, когда вы хотите, чтобы ваше обещание было успешным, и reject используется, когда вы хотите, чтобы оно потерпело неудачу. Это методы, которые принимают аргумент, как показано ниже.
